#!/bin/bash
# #---------------init for item_menu_information_install menu-------------------------------
echo_check
export item_menu_install_myoffice="Установить Мой Офис Документы 2.6.0"
export item_menu_install_finereader="Установить FineReader"
export item_menu_install_crypto1290="Установить Криптопро.v1290"
export item_menu_install_crypto1145="Установить Криптопро.v1145"
export item_menu_install_pdf_editor="Установить master-pdf-editor"
export item_menu_install_telegram="<span weight='bold' foreground='red'>Жирный красный текст</span>"
export item_menu_install_whatsapp="Установить whatsapp"
export item_menu_install_wps="Установить wps-office"
export item_menu_install_notepadplus="Установить notepadplus"
export item_menu_install_yandex="Установить yandex-browser-stable"
export item_menu_install_remina="Установить remmina" 
export item_menu_install_xrdp="Установить xrdp"
export item_menu_install_scan="Установить fly-scan"
export item_menu_install_apps=( 
"\"$item_menu_install_myoffice\""
"\"$item_menu_install_finereader\""  
"\"$item_menu_install_pdf_editor\""  
"\"$item_menu_install_crypto1290\"" 
"\"$item_menu_install_crypto1145\""    
"\"$item_menu_install_telegram\""  
"\"$item_menu_install_whatsapp\"" 
"\"$item_menu_install_wps\""  
"\"$item_menu_install_notepadplus\"" 
"\"$item_menu_install_yandex\"" 
"\"$item_menu_install_remina\"" 
"\"$item_menu_install_xrdp\"" 
"\"$item_menu_install_scan\"" 
"\"$exit_menu\"" 
"\"$exit_app\"")
 
